...
function Attendance({ students, setStudents, addToast }) {
    ...
    var [filterSem, setFilterSem] = useState('');
    var [selectedSubject, setSelectedSubject] = useState(null);
    ...
    activeStudents.forEach(function (s) {
        (s.currentSubjects || []).forEach(function (sub) {
            if (!subjectMap[sub._id]) subjectMap[sub._id] = sub;
        });
    });
    var availableSubjects = Object.values(subjectMap);
    ...
    useEffect(function () {
        ...
        async function load() {
            try {
                var res = await axios.get(API + '/attendance/' + date + '/' + selectedSubject._id);
                var recs = {};
    ...
    function setRecord(studentId, status) {
        var newRecs = Object.assign({}, records);
        newRecs[studentId] = status;
        setRecords(newRecs);
        updateSummary(newRecs);
    }

    var studentsForSubject = selectedSubject
        ? activeStudents.filter(function (s) {
            return (s.currentSubjects || []).some(function (sub) {
                return String(sub._id) === String(selectedSubject._id);
            });
        })
        : activeStudents;
    ...
    function markAllPresent() {
        var newRecs = {};
        studentsForSubject.forEach(function (s) { newRecs[s._id] = 'Present'; });
        setRecords(newRecs);
        updateSummary(newRecs);
    ...
    async function saveAttendance() {
        ...
        try {
            await axios.post(API + '/attendance', {
                ...
                sem: parseInt(filterSem) || (studentsForSubject[0] ? studentsForSubject[0].currentSem : 1),
                department: filterDept || (studentsForSubject[0] ? studentsForSubject[0].department : ''),
    ...
    return (
        <div>
            ...
            <div className="flex items-center gap-3 mb-5 flex-wrap">
                ...
                <select value={filterSem} onChange={function (e) { setFilterSem(e.target.value); }}
                    className="bg-dark-700 border border-dark-500/40 text-dark-50 text-xs px-3 py-2 rounded-lg outline-none focus:border-accent cursor-pointer">
                    <option value="">All Semesters</option>
                    {[1, 2, 3, 4, 5, 6, 7, 8].map(function (n) { return <option key={n} value={n}>Sem {n}</option>; })}
                </select>
            ...
            {/* Subject Selector */}
            <div className="mb-5">
                <div className="text-[10px] font-bold text-dark-300 uppercase tracking-wider mb-2">Select Subject</div>
                {availableSubjects.length === 0 ? (
                    <div className="bg-dark-700 border border-dark-500/30 rounded-xl p-5 text-center text-xs text-dark-300">
                        ...
                        {activeStudents.length === 0
                            ...
                            : 'No subjects found. Add subjects via Academic Details on the Students page.'}
                    </div>
                ) : (
                    <div className="flex flex-wrap gap-2">
                        {availableSubjects.map(function (sub) {
                            var isSelected = selectedSubject && String(selectedSubject._id) === String(sub._id);
                            return (
                                <button key={sub._id}
                                    onClick={function () { setSelectedSubject(isSelected ? null : sub); }}
                                    className={"px-3 py-2 rounded-lg text-xs font-semibold border transition-all " +
                                        (isSelected ? 'bg-accent/15 text-accent border-accent/40' : 'bg-dark-700 border-dark-500/40 text-dark-300 hover:border-accent hover:text-accent')}>
                                    {sub.name}
            ...
            <div className="bg-dark-700 border border-dark-500/30 rounded-xl overflow-hidden">
                {selectedSubject && (
                    <div className="px-5 py-3 border-b border-dark-500/30 flex items-center gap-2">
                        ...
                        <span className="ml-auto text-[10px] text-dark-300">{selectedSubject.credits} credits · {studentsForSubject.length} students</span>
                    </div>
                )}
                <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                        ...
                        <tbody className="divide-y divide-dark-500/20">
                            ...
                            ) : studentsForSubject.map(function (s) {
                                return (
                                    <tr key={s._id} className="hover:bg-accent/[.02] transition-colors">
                                        ...
                                        <td className="px-4 py-3">
                                            ...
                                            <div className="text-[10px] text-dark-300 mt-0.5">Sem {s.currentSem || 1}</div>
                                        </td>
                                        <td className="px-4 py-3">
                                            <div className="flex gap-2">
                                                {['Present', 'Absent', 'Late'].map(function (status) {
                                                    return (
                                                        <button key={status} onClick={function () { setRecord(s._id, status); }}
